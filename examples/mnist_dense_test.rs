...
use autograph::{ArcTensor, Cpu, WebGpu, Device, RwTensor, Tensor};
...
fn main() {
    ...
    let device = Device::from(WebGpu::new(0));
    
    let mut w = Parameter::new(RwTensor::from_shape_vec(
        &device,
        [10, 28 * 28],
        &*w.value().read().unwrap().as_slice()
    ));
    // Create a trainable bias Parameter filled with zeros
    let mut b = Parameter::new(RwTensor::from_shape_vec(
        &device, 
        10, 
        &*b.value().read().unwrap().as_slice()
    ));
